...
public class ClientFrame extends JFrame implements Runnable {
    ...
    public ClientFrame(String name) {
        ...
        itemLeaveRoom.addActionListener(ae -> { // bạn có muốn thoát khỏi room k
        ...
        itemChangePass.addActionListener(ae -> {
        });
        itemLogout.addActionListener(ae -> { // logout, bạn có muốn đăng xuất k
        ...
        itemSendFile.addActionListener(ae -> { // thông báo chát riêng mới gửi file dc.
    ...
    private void addEventsForWelcomePanel() { // giao diện welcome cả signup và login

        welcomePanel.getBtLogin_welcome().addActionListener(ae -> {
        ...
        welcomePanel.getBtSignUp_welcome().addActionListener(ae -> {
            welcomePanel.setVisible(false);
    ...
    private void addEventsForSignUpPanel() { // giao diện signup
    ...
    private void addEventsForLoginPanel() { // giao diện login
    ...
    private void addEventsForClientPanel() { // có tạo ra 2 buton send và exit
        clientPanel.getBtSend().addActionListener(ae -> btSendEvent());
        clientPanel.getBtExit().addActionListener(ae -> btExitEvent());
    ...
    private void openPrivateChatInsideRoom() {
        ...
        if (timeClicked == 2) {  //nếu như countingTo500ms chưa thực hiện xong, tức là timeClicked vẫn = 2:
            ...
            if (!listReceiver.containsKey(nameClicked)) {    //nếu đây là lần đầu tiên nhắn tin với người mà ta vừa double-click vào
                ...
                pc.getLbReceiver().setText("Private chat with \"" + pc.receiver + "\""); // chat riêng
