...
public class ServerThread extends Thread {
    ...
    */
    ...
    public ServerThread(Socket socketOfServer) { // luồng thread
    ...
    public void appendMessage(String message) { // cộng thêm kí tự  vào chuỗi tin nhắn.
    ...
    public void notifyToAllUsers(String message) { // thông báo gửi tn cho tất cả user
    ...
    public void notifyToUsersInRoom(String message) { // thông báo đến các client trong room
        Enumeration<ServerThread> clients = listUser.elements();
        ServerThread st;
        BufferedWriter writer;
    ...
    public void notifyToUsersInRoom(String room, String message) {      //gửi bản tin message tới các client trong phòng room
        Enumeration<ServerThread> clients = listUser.elements();
        ServerThread st;
        BufferedWriter writer;
    ...
    public String getAllUsers() { // nhận kết quả cho tất cả listuser
    ...
    public String getUsersThisRoom() { // kết quả trả về có trong listuser có key là tên dc cách bằng 1 dấu cách. ví dụ là |huy|
        StringBuffer kq = new StringBuffer();
        String temp ;
        ServerThread st;
        // duyệt danh sách user <listUser>
        Enumeration<String> keys = listUser.keys();
    ...
    public String getUsersAtRoom(String room) {  // tất cả user trong room nhận dc tin nhắn
        // đầu vào là tên phòng, và tìm kiếm những user trong phòng đó
        StringBuffer kq = new StringBuffer();
        String temp;
        ServerThread st;
        Enumeration<String> keys = listUser.keys();
        ...
        while(keys.hasMoreElements()) {
            temp = keys.nextElement();
            st = listUser.get(temp);
            // check nếu user ở trong phòng cần tìm thì trả ra
    ...
    public void clientQuit() {
        ...
        if(clientName != null) {
            ...
            listUser.remove(clientName); // user vừa xóa. server thông báo k còn ai trong room
    ...
    public void changeUserRoom() {      //cập nhật lại room cho chính đối tượng thuộc class này trong listUser
        ...
        st.clientRoom = this.clientRoom; // cập nhật lại room cho user vừa thay đổi
    ...
    public void removeUserRoom() { // xóa user khỏi room
        ServerThread st = listUser.get(this.clientName);
                ...
                try {
                    ...
                    switch (cmd) {
                        case "CMD_CHAT":
                            ...
                            str = str.delete(0, 9);
                            notifyToUsersInRoom("CMD_CHAT|" + this.clientName+"|"+ str);    //this.clientName = tên client gửi tin tới
                            break;
